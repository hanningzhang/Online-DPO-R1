...
def main(args):
    ...
    DatasetDict({'train': dataset}).push_to_hub("weqweasdas/prompt_math")
    '''
    ######
        ...
        for k in range(len(remain_prompts)):
            ...
            if "gemma" in args.model_name_or_path:
               ...
               # exec_result1 = f"<|eot_id|><|start_header_id|>user<|end_header_id|>\n\nSince your initial response is self-evaluated as incorrect, there might be an error in the solution above because of lack of understanding of the question. Please correct the error, if any, and rewrite the solution. Put your final answer within \\boxed{{}}."
                ...
                raise NotImplementedError(args.prompt_type + "and " + args.model_name_or_path)
            #if rm_score:
            #    query = query + f"<|eot_id|><|start_header_id|>user<|end_header_id|>\n\nThanks" + "<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n"
            #else:
            ...
            if True:
                ...
                if "Is the most recent final answer correct (Yes or No)?" + "<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n" + "Yes" in query:
